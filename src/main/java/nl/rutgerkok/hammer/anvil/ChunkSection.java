...
final class ChunkSection {
    ...
    private static final int TOTAL_SIZE = SECTION_X_SIZE * SECTION_Y_SIZE
            * SECTION_Z_SIZE;
    ...
    private static CompoundTag createChunkSection(CompoundTag chunkTag, int y) {
        ...
        chunkSection.setByteArray(SectionTag.BLOCK_DATA,
                new byte[TOTAL_SIZE_NIBBLE]);
        chunkSection.setByteArray(SectionTag.BLOCK_LIGHT,
                new byte[TOTAL_SIZE_NIBBLE]);
        chunkSection.setByteArray(SectionTag.SKY_LIGHT,
                new byte[TOTAL_SIZE_NIBBLE]);
    ...
    private static CompoundTag getChunkSection(CompoundTag chunkTag, int y) {
        ...
        if (sectionIndex < sections.size()) {
            ...
            if (section != null
                    && section.getByte(SectionTag.INDEX) == sectionIndex) {
                return section;
            }
        }
    ...
    static byte getMaterialData(CompoundTag chunkTag, int x, int y, int z) {
        ...
        byte[] dataArray = section.getByteArray(SectionTag.BLOCK_DATA,
                TOTAL_SIZE_NIBBLE);
     ...
     *             If the x, y or z are out of bounds.
     */
    static final short getMaterialId(CompoundTag chunkTag, int x, int y,
            int z) {
        CompoundTag section = getChunkSection(chunkTag, y);
        if (section == null) {
            return 0;
        ...
        byte[] extBlocks = section.getByteArray(SectionTag.EXT_BLOCK_IDS,
                TOTAL_SIZE_NIBBLE);
    ...
    private static int getPositionInSectionArray(int xInSection, int yInSection,
            int zInSection) {
     ...
     *            The block data.
     */
    static void setMaterialIdAndData(CompoundTag chunkTag, int x, int y, int z,
            short blockId, byte blockData) {
        CompoundTag section = getChunkSection(chunkTag, y);
        if (section == null) {
